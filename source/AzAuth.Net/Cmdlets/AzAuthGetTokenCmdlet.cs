...
public abstract class AzAuthGetTokenCmdlet : AzAuthBaseCmdlet
{
...
{string.Join(Environment.NewLine, CredentialPrecedence.Select(cred => $"{cred} ({TokenManager.GetCredentialDocumentationUrl(cred)})"))}");
        ...
        else if (DeviceCode.IsPresent)
        {
            ...
            var token = TokenManager.GetTokenDeviceCode(Resource, Scope, Claim, ClientId, Tenant, TokenCache, TimeoutSeconds, this, stopProcessing.Token);
